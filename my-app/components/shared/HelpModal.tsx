  ...
  return (
    isopen && (
      <View
        ...
        className="bg-transparent/80 items-center justify-center z-50 absolute ">
        <Animated.View
          className={` p-8 w-[90%] bg-white rounded-md items-center justify-center`}>
          {/* close icon  */}
          <TouchableOpacity
            onPress={() => setIsOpen(false)}
            className="absolute font-bold z-50 top-4 right-4">
            <Ionicons
              className="font-bold"
          ...
          <TouchableOpacity
            onPress={() => {
              return Linking.openURL(`mailto:vivekkumar17091999sept@gmail.com`);
            }}
            className="flex-row items-center bg-gray-200 w-full p-2 rounded-md">
            <Octicons name="mail" size={22} />
            <Text className="ml-4 font-semibold text-lg" numberOfLines={1}>
              help@hostlio.in
            </Text>
          </TouchableOpacity>
