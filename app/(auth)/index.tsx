...
const MainAuthScreen = () => {
  ...
  const { isLoading, isSuccess, isError, user, errMSg } = useSelector(
    (state: RootState) => state.auth
  );
  const [email, setemail] = useState<string>("");
  const [password, setpassword] = useState<string>("");
  const [forgotPassModalState, setforgotPassModalState] =
  ...
  const handleLoginFn = async () => {
    ...
    if (!email.includes("@") || email.length === 0) {
      return showToast(
        ...
        "error",
        "pleae enter correct email address."
      );
    }
    ...
    <AuthContainer verticallyCenter={true}>
      ...
      {forgotPassModalState && (
        <ForgotPasswordModal
          ...
          isOpen={forgotPassModalState}
          setIsOpen={setforgotPassModalState as any}
        />
      )}

      {isLoading && <Loader loadingMessage="Please wait..." />}
      <View className="bg-White p-4 rounded-lg shadow-md shadow-black/50 ">
        <View className="mb-4">
        ...
        <View className="w-full flex-row items-center justify-end">
          <Text
            ...
            className="border-b-2 border-b-gray-200 pb-1 text-Primary">
            Forgot Password
          </Text>
        </View>
        ...
        <View className="flex-row w-full items-center justify-center mt-8">
          ...
          <Text
            ...
            className="underline text-Primary text-2xl font-medium ml-2 text-md">
            Register
          </Text>
        </View>
      </View>
