...
const Button = ({ loading, lable, className, onPress, Icon, outline, disabled }: Props) => {
    ...
    return (
        <TouchableOpacity disabled={disabled} className={`py-3 my-2 bg-Primary justify-center rounded-md ${Icon && "px-8"} flex-row items-center w-full ${outline && "bg-transparent border-2 border-Primary"} ${disabled && "bg-Gray"}`} onPress={() => onPress()}>
